...
countDiagonal = sum . map countString . concatMap getDiagonals . concatMap transpose . windows 4 . map (windows 4)
...
masx :: [String] -> Int
masx = length . filter isMasx . map getDiagonals . concatMap transpose . windows 3 . map (windows 3)
