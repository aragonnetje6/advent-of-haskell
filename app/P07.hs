...
operantOptions operators n =
  concatMap
    (\op -> map (op :) (operantOptions operators (n - 1)))
    operators

isPossible :: [Operator] -> Equation -> Bool
isPossible operators eq =
  any (satisfies eq) $
    operantOptions operators $
      toInteger (length $ operands eq) - 1
...
part2 =
  show
    . sum
    . map target
    . filter (isPossible [(+), (*), concatInteger])
    . unwrap
    . parse parseFile ""
